...
void FreeCam::PriorityUpdate(_float fTimeDelta)
{
    ...
    {
        _long MouseMoveX = m_pEngineUtility->GetMouseMove(MOUSEMOVESTATE::MOUSEMOVE_X);
        if (MouseMoveX != 0)
            pTransform->RotateTimeDelta(XMVectorSet(0.f, 1.f, 0.f, 0.f), MouseMoveX * m_fSensor * fTimeDelta);

        _vector vLook = XMVector4Normalize(pTransform->GetState(MATRIXROW_LOOK));

        _float lookAngle = asinf(XMVectorGetY(vLook));
        constexpr _float limitAngle = XMConvertToRadians(85.f);

        _long MouseMoveY = m_pEngineUtility->GetMouseMove(MOUSEMOVESTATE::MOUSEMOVE_Y);
        if (MouseMoveY != 0)
        {
...
{
    _vector vPos = static_cast<Transform*>(FindComponent(TEXT("Transform")))->GetState(MATRIXROW_POSITION);
    m_pEngineUtility->SetActiveLightsByDistance(vPos, 50.f, 4);
}
