...
HouseDoorHandle::HouseDoorHandle()
	: PartTemplate{}
{
}

HouseDoorHandle::HouseDoorHandle(const HouseDoorHandle& Prototype)
	: PartTemplate{ Prototype }
{
}

void HouseDoorHandle::LateUpdate(_float fTimeDelta)
{
...
HRESULT HouseDoorHandle::Render()
{
...
HouseDoorHandle* HouseDoorHandle::Create()
{
    HouseDoorHandle* pInstance = new HouseDoorHandle();
    if (FAILED(pInstance->InitializePrototype()))
    {
        MSG_BOX("Failed to Created : HouseDoorHandle");
        SafeRelease(pInstance);
    }
    return pInstance;
}

Object* HouseDoorHandle::Clone(void* pArg)
{
    PART_DESC partDesc{};
    Transform* pTransform = dynamic_cast<Transform*>(FindComponent(TEXT("Transform")));
    ...
    HouseDoorHandle* pInstance = new HouseDoorHandle(*this);
...
HRESULT HouseDoorHandle::ReadyComponents()
{
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Shader_VtxMesh"), TEXT("Shader"), nullptr, nullptr)))
        return E_FAIL;
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_HouseDoorHandle"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;
