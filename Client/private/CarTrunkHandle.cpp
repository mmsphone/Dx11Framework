...
CarTrunkHandle::CarTrunkHandle()
	: PartTemplate{}
{
}

CarTrunkHandle::CarTrunkHandle(const CarTrunkHandle& Prototype)
	: PartTemplate{ Prototype }
{
}

void CarTrunkHandle::LateUpdate(_float fTimeDelta)
{
...
CarTrunkHandle* CarTrunkHandle::Create()
{
    CarTrunkHandle* pInstance = new CarTrunkHandle();
    if (FAILED(pInstance->InitializePrototype()))
    {
        MSG_BOX("Failed to Created : CarTrunkHandle");
        SafeRelease(pInstance);
    }
    return pInstance;
}

Object* CarTrunkHandle::Clone(void* pArg)
{
    PART_DESC partDesc{};
    Transform* pTransform = dynamic_cast<Transform*>(FindComponent(TEXT("Transform")));
    ...
    CarTrunkHandle* pInstance = new CarTrunkHandle(*this);
...
HRESULT CarTrunkHandle::ReadyComponents()
{
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Shader_VtxMesh"), TEXT("Shader"), nullptr, nullptr)))
        return E_FAIL;
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_CarTrunkHandle"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;
