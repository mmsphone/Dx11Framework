...
WoodenFence::WoodenFence(const WoodenFence& Prototype)
    : ObjectTemplate{ Prototype }
{
}

void WoodenFence::LateUpdate(_float fTimeDelta)
{
...
HRESULT WoodenFence::ReadyComponents()
{
    /* For.Com_Shader */
    ...
    /* For.Com_Model */
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_WoodenFence"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;

    return S_OK;
}

WoodenFence* WoodenFence::Create()
{
    WoodenFence* pInstance = new WoodenFence();

    if (FAILED(pInstance->InitializePrototype()))
    {
        MSG_BOX("Failed to Created : WoodenFence");
...
Object* WoodenFence::Clone(void* pArg)
{
    WoodenFence* pInstance = new WoodenFence(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned : WoodenFence");
        SafeRelease(pInstance);
    }
