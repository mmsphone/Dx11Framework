...
Plant::Plant(const Plant& Prototype)
    : ObjectTemplate{ Prototype }
{
}

void Plant::LateUpdate(_float fTimeDelta)
{
...
HRESULT Plant::ReadyComponents()
{
    ...
    /* For.Com_Model */
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_Plant"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;
...
Plant* Plant::Create()
{
    Plant* pInstance = new Plant();

    if (FAILED(pInstance->InitializePrototype()))
    {
        MSG_BOX("Failed to Created : Plant");
...
Object* Plant::Clone(void* pArg)
{
    Plant* pInstance = new Plant(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned : Plant");
        SafeRelease(pInstance);
    }
