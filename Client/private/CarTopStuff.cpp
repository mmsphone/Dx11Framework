...
CarTopStuff::CarTopStuff()
	: PartTemplate{}
{
}

CarTopStuff::CarTopStuff(const CarTopStuff& Prototype)
	: PartTemplate{ Prototype }
{
}

void CarTopStuff::LateUpdate(_float fTimeDelta)
{
...
CarTopStuff* CarTopStuff::Create()
{
    CarTopStuff* pInstance = new CarTopStuff();
    if (FAILED(pInstance->InitializePrototype()))
    {
        MSG_BOX("Failed to Created : CarTopStuff");
        SafeRelease(pInstance);
    }
    return pInstance;
}

Object* CarTopStuff::Clone(void* pArg)
{
    PART_DESC partDesc{};
    Transform* pTransform = dynamic_cast<Transform*>(FindComponent(TEXT("Transform")));
    ...
    CarTopStuff* pInstance = new CarTopStuff(*this);
...
HRESULT CarTopStuff::ReadyComponents()
{
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Shader_VtxMesh"), TEXT("Shader"), nullptr, nullptr)))
        return E_FAIL;
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_CarTopStuff"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;
