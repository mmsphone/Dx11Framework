...
Ground::Ground(const Ground& Prototype)
    : ObjectTemplate{ Prototype }
{
}

void Ground::LateUpdate(_float fTimeDelta)
{
...
HRESULT Ground::ReadyComponents()
{
    /* For.Com_Shader */
    ...
    /* For.Com_Model */
    if (FAILED(AddComponent(SCENE::GAMEPLAY, TEXT("Model_Ground"), TEXT("Model"), nullptr, nullptr)))
        return E_FAIL;
...
Object* Ground::Clone(void* pArg)
{
    Ground* pInstance = new Ground(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned : Ground");
        SafeRelease(pInstance);
    }
